    ...
    <div class="space-y-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">In-app announcements and updates.</p>
            </div>
            <form method="POST" action="{{ route('creator.notifications.read_all') }}">
                @csrf
                @method('PATCH')
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <div class="mt-1 text-sm text-slate-600">Enable to receive updates on this device.</div>
                </div>
                <button type="button"
                ...
                @foreach($items as $n)
                    <div class="border-b border-slate-100 px-5 py-4 last:border-b-0">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <form method="POST" action="{{ route('creator.notifications.read', $n) }}">
                                @csrf
                                @method('PATCH')
