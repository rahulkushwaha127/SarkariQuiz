...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400&family=Lato:wght@400;700&display=swap" rel="stylesheet">
    <style>
        .font-display { font-family: 'Cormorant Garamond', Georgia, serif; }
        .font-body    { font-family: 'Lato', ui-sans-serif, system-ui, sans-serif; }
        ...
        .text-gradient-classic {
            background: linear-gradient(135deg, #1e3a5f 0%, #0f172a 50%, #b45309 100%);
        ...
        .hover-lift:hover { transform: translateY(-3px); box-shadow: 0 12px 35px rgba(15, 23, 42, 0.12); }

        .section-divider-classic {
            width: 64px;
            height: 3px;
            background: linear-gradient(90deg, #b45309, #ca8a04, #eab308);
            margin: 0 auto;
        }
        ...
        .classic-hero-bg {
            background: linear-gradient(160deg, #0f172a 0%, #1e3a5f 30%, #1e293b 60%, #0f172a 100%);
        }
        .classic-gold-glow {
            background: radial-gradient(ellipse 70% 50% at 50% 0%, rgba(202, 138, 4, 0.18) 0%, rgba(30, 58, 95, 0.1) 50%, transparent 70%);
        }
...
<body class="min-h-screen bg-stone-50 text-slate-800 antialiased font-body">
    @php
    ...
    <header class="sticky top-0 z-50 border-b border-stone-200 bg-white/95 backdrop-blur-sm">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
            <a href="{{ url('/') }}" class="flex items-center gap-2.5">
                <div class="grid h-9 w-9 place-items-center rounded-lg border border-amber-700/30 bg-gradient-to-b from-slate-800 to-slate-900 text-sm font-bold text-amber-400 shadow">Q</div>
                <span class="text-sm font-semibold tracking-tight text-slate-800">{{ $siteName ?? config('app.name', 'QuizWhiz') }}</span>
            </a>
            <nav class="flex items-center gap-3">
                @if($hasWhatsapp)
                    <a href="{{ $waLink }}" target="_blank" rel="noopener" class="hidden items-center gap-2 rounded border border-[#25D366] bg-[#25D366] px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-[#20BD5A] sm:inline-flex">
                ...
                @endif
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded border border-stone-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:bg-stone-50">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="rounded border border-stone-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:bg-stone-50">Login</a>
                @endauth
            </nav>
        </div>
    </header>
    ...
    <section class="relative overflow-hidden">
        @if ($hasCover)
            ...
            <div class="absolute inset-0 bg-gradient-to-b from-slate-900/85 via-slate-900/70 to-slate-900/90"></div>
        @else
            <div class="classic-hero-bg absolute inset-0"></div>
            <div class="classic-gold-glow absolute inset-0"></div>
        @endif
            ...
            <div class="flex flex-col items-center text-center sm:flex-row sm:items-end sm:text-left sm:gap-8">
                <div class="mb-6 shrink-0 sm:mb-0">
                    @if ($hasAvatar)
                        <img src="{{ asset('storage/' . $avatarPath) }}" alt="{{ $creator->name }}"
                             class="h-28 w-28 rounded-xl border-4 border-amber-500/40 object-cover shadow-2xl sm:h-36 sm:w-36 md:h-40 md:w-40"
                             loading="eager" width="160" height="160" />
                    @else
                        <div class="flex h-28 w-28 items-center justify-center rounded-xl border-4 border-amber-500/40 bg-gradient-to-b from-slate-700 to-slate-900 text-4xl font-bold text-amber-400 shadow-2xl sm:h-36 sm:w-36 sm:text-5xl md:h-40 md:w-40">{{ $initials }}</div>
                    @endif
                </div>
                <div class="min-w-0 flex-1">
                    ...
                    @if ($v('about', 'tagline') && $tagline)
                        <p class="mt-3 text-base text-amber-100/90 sm:text-lg md:text-xl max-w-2xl">{{ $tagline }}</p>
                    @endif
                    <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4 sm:mt-8">
                        @if ($hasWhatsapp)
                            <a href="{{ $waLink }}" target="_blank" rel="noopener"
                               class="inline-flex items-center justify-center gap-2.5 rounded border-2 border-[#25D366] bg-[#25D366] px-6 py-3 text-sm font-bold text-white shadow-lg transition hover:bg-[#20BD5A] active:scale-[0.98]">
                        ...
                        @if ($hasContact && !$hasWhatsapp)
                            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $centerContact) }}"
                               class="inline-flex items-center justify-center gap-2 rounded border-2 border-amber-500/50 bg-white px-6 py-3 text-sm font-bold text-slate-800 shadow-lg transition hover:bg-amber-50 active:scale-[0.98]">
                        ...
                        @if ($hasWebsite)
                            <a href="{{ $centerWebsite }}" target="_blank" rel="noopener"
                               class="inline-flex items-center justify-center rounded border-2 border-amber-400/50 px-6 py-3 text-sm font-bold text-white backdrop-blur-sm transition hover:bg-amber-500/20 active:scale-[0.98]">
                                Visit website
                            </a>
                        @endif
                    </div>
        ...
        <div class="absolute bottom-0 left-0 right-0">
            <svg viewBox="0 0 1440 60" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" class="block h-[30px] w-full sm:h-[40px] md:h-[60px]">
                <path d="M0 60h1440V30C1200 55 960 0 720 30S240 55 0 30v30z" fill="#fafaf9"/>
            </svg>
        </div>
    </section>

    {{-- STATS BAR --}}
    <section class="relative z-10 -mt-1 bg-stone-50">
        <div class="mx-auto max-w-4xl px-5 sm:px-8">
            <div class="classic-card flex items-center justify-center gap-6 rounded-lg border border-stone-200 bg-white px-6 py-5 sm:gap-10 sm:px-10 sm:py-6 -mt-6 sm:-mt-8">
                <div class="text-center">
                    <div class="font-display text-2xl font-bold text-slate-800 sm:text-3xl">{{ $stats['public_quizzes'] ?? 0 }}</div>
                    <div class="mt-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Quizzes</div>
                </div>
                <div class="h-10 w-px bg-stone-200"></div>
                <div class="text-center">
                    <div class="font-display text-2xl font-bold text-slate-800 sm:text-3xl">{{ $stats['public_questions'] ?? 0 }}</div>
                    <div class="mt-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Questions</div>
                </div>
                @if (count($selectedStudents) > 0)
                    <div class="h-10 w-px bg-stone-200"></div>
                    <div class="text-center">
                        <div class="font-display text-2xl font-bold text-amber-800 sm:text-3xl">{{ count($selectedStudents) }}+</div>
                        <div class="mt-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Selections</div>
                    </div>
                @endif
                @if (count($faculty) > 0)
                    <div class="hidden h-10 w-px bg-stone-200 sm:block"></div>
                    <div class="hidden text-center sm:block">
                        <div class="font-display text-2xl font-bold text-slate-800 sm:text-3xl">{{ count($faculty) }}</div>
                        <div class="mt-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Faculty</div>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if ($v('about') && $v('about', 'bio') && $bio)
        <section class="fade-up border-t border-stone-200 bg-white py-16 sm:py-20">
            <div class="mx-auto max-w-3xl px-5 text-center sm:px-8">
                <span class="inline-block border-b-2 border-amber-600 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-800">About us</span>
                <h2 class="mt-6 font-display text-2xl font-bold text-slate-900 sm:text-3xl md:text-4xl">
                    @if ($v('institute','name') && $centerName) {{ $centerName }} @else Know more about us @endif
                </h2>
                <div class="mt-4 section-divider-classic"></div>
    ...
    @if (in_array('courses', $instFields))
        <section class="fade-up border-t border-stone-200 bg-stone-50/80 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-block border-b-2 border-amber-600 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-800">What we offer</span>
                    <h2 class="mt-6 font-display text-2xl font-bold text-slate-900 sm:text-3xl">Courses &amp; programmes</h2>
                    <div class="mt-4 section-divider-classic"></div>
                </div>
                <div class="mt-10 flex flex-wrap justify-center gap-3 sm:gap-4">
                    @foreach (preg_split('/[\n,]+/', $coursesOffered) as $course)
                        ...
                        @if ($course !== '')
                            <div class="classic-card rounded-lg border border-stone-200 bg-white px-5 py-3.5 text-sm font-semibold text-slate-700 transition hover:border-amber-700/30 sm:px-6 sm:py-4 sm:text-base">
                                {{ $course }}
                            </div>
                        @endif
                    @endforeach
    ...
    @if ($v('faculty') && count($faculty) > 0)
        <section class="fade-up border-t border-stone-200 bg-white py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-block border-b-2 border-amber-600 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Our team</span>
                    <h2 class="mt-6 font-display text-2xl font-bold text-slate-900 sm:text-3xl">Expert faculty</h2>
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($faculty as $member)
                        <div class="hover-lift classic-card group rounded-lg border border-stone-200 bg-white p-6 text-center sm:p-8">
                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-lg border-2 border-amber-700/30 bg-gradient-to-b from-slate-700 to-slate-800 text-2xl font-bold text-amber-400 transition group-hover:border-amber-600/50">
                                {{ mb_substr($member['name'] ?? '', 0, 1) ?: '?' }}
                            </div>
                            <div class="mt-4">
                                <h3 class="font-display text-lg font-bold text-slate-800">{{ $member['name'] ?? '' }}</h3>
                                @if (!empty($member['role']))
                                    <p class="mt-1 text-sm font-semibold text-amber-800">{{ $member['role'] }}</p>
                                @endif
                                @if (!empty($member['bio']))
    ...
    @if ($v('achievers') && count($selectedStudents) > 0)
        <section class="fade-up relative overflow-hidden border-t border-stone-200 py-16 sm:py-20 classic-hero-bg">
            <div class="classic-gold-glow absolute inset-0"></div>
            <div class="relative mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-block border-b-2 border-amber-400 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-200">Success stories</span>
                    <h2 class="mt-6 font-display text-2xl font-bold text-white sm:text-3xl md:text-4xl">Our achievers</h2>
                    <p class="mt-2 text-sm text-slate-300 sm:text-base">Students who made us proud.</p>
                    <div class="mt-4 section-divider-classic"></div>
                </div>
                <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-5">
                    @foreach ($selectedStudents as $student)
                        <div class="hover-lift group flex items-center gap-4 rounded-lg border border-amber-500/25 bg-white/10 px-5 py-4 backdrop-blur-sm transition hover:bg-white/15 sm:px-6 sm:py-5">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-amber-400/40 bg-amber-500/20 text-amber-300 transition group-hover:bg-amber-500/30">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="font-display font-bold text-white">{{ $student['name'] ?? '' }}</div>
    ...
    @if ($v('images') && $v('images', 'gallery') && count($galleryImages) > 0)
        <section class="fade-up border-t border-stone-200 bg-white py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-block border-b-2 border-amber-600 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Life at our institute</span>
                    <h2 class="mt-6 font-display text-2xl font-bold text-slate-900 sm:text-3xl">Gallery</h2>
                ...
                <div class="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5">
                    @foreach ($galleryImages as $path)
                        @if (\Illuminate\Support\Facades\Storage::disk('public')->exists($path))
                            <div class="gallery-item group overflow-hidden rounded-lg border border-stone-200 transition hover:shadow-lg">
    ...
    @if (count($instFields) > 0)
        <section class="fade-up border-t border-stone-200 bg-stone-50/80 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-block border-b-2 border-amber-600 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Get in touch</span>
                    <h2 class="mt-6 font-display text-2xl font-bold text-slate-900 sm:text-3xl">Visit or contact us</h2>
                    <div class="mt-4 section-divider-classic"></div>
                </div>
                <div class="mx-auto mt-10 max-w-3xl overflow-hidden rounded-lg border border-stone-200 bg-white shadow-lg">
                    <div class="grid gap-0 md:grid-cols-2">
                        <div class="border-b border-stone-100 p-6 sm:p-8 md:border-b-0 md:border-r md:border-stone-200">
                            <dl class="space-y-5">
                                @if (in_array('name', $instFields))
                                    <div class="flex items-start gap-3">
                                        <div class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded border border-amber-700/20 bg-amber-50 text-amber-800">
                                ...
                                @if (in_array('contact', $instFields))
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div><dt class="text-xs font-semibold uppercase tracking-wider text-slate-400">Contact</dt><dd class="mt-0.5"><a href="tel:{{ preg_replace('/[^0-9+]/', '', $centerContact) }}" class="font-semibold text-amber-800 hover:text-amber-900 hover:underline">{{ $centerContact }}</a></dd></div>
                                    </div>
                                @endif
                            </dl>
                        </div>
                        <div class="flex flex-col items-center justify-center gap-4 border-t border-stone-100 bg-amber-50/50 p-6 sm:p-8 md:border-t-0 md:border-l-0">
                            <div class="text-center">
                            ...
                            <div class="flex flex-col gap-3 w-full max-w-[220px]">
                                @if ($hasWhatsapp)
                                    <a href="{{ $waLink }}" target="_blank" rel="noopener"
                                       class="inline-flex items-center justify-center gap-2 rounded border-2 border-[#25D366] bg-[#25D366] px-5 py-3 text-sm font-bold text-white shadow transition hover:bg-[#20BD5A]">
                                        <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.865 9.865 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                                        WhatsApp
                                    </a>
                                @endif
                                @if ($hasContact)
                                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', $centerContact) }}"
                                       class="inline-flex items-center justify-center gap-2 rounded border-2 border-stone-300 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-stone-50">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                                        Call now
                                    </a>
                                @endif
    ...
    <section class="fade-up border-t border-stone-200 bg-white py-16 sm:py-20">
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
            <div class="text-center">
                <span class="inline-block border-b-2 border-amber-600 pb-1 text-xs font-semibold uppercase tracking-wider text-amber-800">Practice now</span>
                <h2 class="mt-6 font-display text-2xl font-bold text-slate-900 sm:text-3xl">Public quizzes</h2>
                <p class="mt-2 text-sm text-slate-500 sm:text-base">Practice with {{ $creator->name }}'s quizzes.</p>
                <div class="mt-4 section-divider-classic"></div>
            </div>
            @if ($publicQuizzes->count() === 0)
                <div class="mx-auto mt-10 max-w-md rounded-lg border-2 border-dashed border-stone-200 bg-stone-50 p-10 text-center">
                    <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-lg border border-stone-200 bg-white">
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($publicQuizzes as $quiz)
                        <a href="{{ route('public.quizzes.show', $quiz->unique_code) }}"
                           class="hover-lift classic-card group flex flex-col rounded-lg border border-stone-200 bg-white p-5 text-left transition hover:border-amber-700/30 sm:p-6">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0 flex-1">
                                    <h3 class="font-display font-bold text-slate-800 break-words group-hover:text-amber-900 leading-snug">{{ $quiz->title }}</h3>
                                    @if ($quiz->description)
                                        <p class="mt-1.5 line-clamp-2 text-sm leading-relaxed text-slate-500">{{ $quiz->description }}</p>
                                    @endif
                                </div>
                                <span class="shrink-0 rounded border border-amber-700/30 bg-amber-50 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-amber-800">{{ $quiz->mode }}</span>
                            </div>
                            <div class="mt-auto pt-4 flex items-center justify-between border-t border-stone-100">
                                <span class="text-xs font-medium text-slate-500">{{ $quiz->questions_count ?? 0 }} questions</span>
                                <span class="inline-flex items-center gap-1 text-sm font-bold text-amber-800 group-hover:text-amber-900">
                                    Play
    ...
    @if ($v('social') && !empty($social))
        <section class="border-t border-stone-200 bg-stone-50 py-12">
            <div class="mx-auto max-w-6xl px-5 text-center sm:px-8">
                <h3 class="text-sm font-semibold uppercase tracking-wider text-slate-500">Follow us on</h3>
                <div class="mt-4 flex flex-wrap justify-center gap-3">
                    @foreach ($social as $label => $url)
                        @if (is_string($url) && $url !== '')
                            <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                               class="inline-flex items-center gap-2 rounded border border-stone-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-amber-700/40 hover:text-amber-800 active:scale-[0.97]">
                                @php $lbl = is_string($label) && $label !== '' ? strtolower($label) : 'link'; @endphp
        ...
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
            <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                <div class="flex items-center gap-2">
                    <div class="grid h-7 w-7 place-items-center rounded border border-amber-700/30 bg-gradient-to-b from-slate-800 to-slate-900 text-xs font-bold text-amber-400">Q</div>
