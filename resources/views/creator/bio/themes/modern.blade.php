...
<head>
    ...
    <style>
        ...
        .text-gradient-royal {
            background: linear-gradient(135deg, #c084fc 0%, #f59e0b 50%, #fbbf24 100%);
        ...
        .hover-lift:hover { transform: translateY(-4px); box-shadow: 0 20px 50px rgba(88, 28, 135, 0.2); }
        ...
        .royal-glow {
            background: radial-gradient(ellipse 80% 60% at 50% 0%, rgba(217, 119, 6, 0.2) 0%, rgba(124, 58, 237, 0.15) 40%, transparent 70%);
        }
        .royal-card-gradient {
            background: linear-gradient(145deg, rgba(255,255,255,0.95) 0%, rgba(245, 243, 255, 0.98) 100%);
        }
    </style>
</head>
<body class="min-h-screen bg-slate-50 text-slate-800 antialiased font-body">
    @php
    ...
    {{-- NAVBAR --}}
    <header class="sticky top-0 z-50 border-b border-violet-200/50 bg-white/90 backdrop-blur-xl">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
            <a href="{{ url('/') }}" class="flex items-center gap-2.5">
                <div class="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-violet-600 via-purple-600 to-amber-500 text-sm font-bold text-white shadow-lg shadow-violet-500/30">Q</div>
            ...
            <nav class="flex items-center gap-3">
                ...
                @endif
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-full border border-violet-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:bg-violet-50">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="rounded-full border border-violet-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:bg-violet-50">Login</a>
                @endauth
            </nav>
        </div>
    </header>
    ...
    <section class="relative overflow-hidden">
        @if ($hasCover)
            ...
            <div class="absolute inset-0 bg-gradient-to-b from-[#1e1b4b]/85 via-[#312e81]/75 to-[#1e1b4b]/90"></div>
        @else
            <div class="royal-gradient-hero absolute inset-0"></div>
            <div class="royal-glow absolute inset-0"></div>
        @endif
                ...
                <div class="mb-6 shrink-0 sm:mb-0">
                    @if ($hasAvatar)
                        <img src="{{ asset('storage/' . $avatarPath) }}" alt="{{ $creator->name }}"
                             class="h-28 w-28 rounded-2xl border-4 border-white/30 border-amber-400/20 object-cover shadow-2xl ring-2 ring-amber-400/30 sm:h-36 sm:w-36 sm:rounded-3xl md:h-40 md:w-40"
                             loading="eager" width="160" height="160" />
                    @else
                        <div class="flex h-28 w-28 items-center justify-center rounded-2xl border-4 border-white/30 border-amber-400/20 bg-gradient-to-br from-violet-500 via-purple-500 to-amber-500 text-4xl font-bold text-white shadow-2xl sm:h-36 sm:w-36 sm:rounded-3xl sm:text-5xl md:h-40 md:w-40">{{ $initials }}</div>
                    @endif
                </div>
                    ...
                    <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4 sm:mt-8">
                        ...
                        @if ($hasContact && !$hasWhatsapp)
                            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $centerContact) }}"
                               class="inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-bold text-slate-800 shadow-lg transition hover:bg-amber-50 active:scale-[0.97]">
                        ...
                        @if ($hasWebsite)
                            <a href="{{ $centerWebsite }}" target="_blank" rel="noopener"
                               class="inline-flex items-center justify-center rounded-full border-2 border-amber-400/50 bg-white/10 px-6 py-3 text-sm font-bold text-white backdrop-blur-sm transition hover:bg-amber-400/20 hover:border-amber-400 active:scale-[0.97]">
                                Visit website
                            </a>
                        @endif
                    </div>
    ...
    {{-- STATS BAR --}}
    <section class="relative z-10 -mt-1 bg-slate-50">
        <div class="mx-auto max-w-4xl px-5 sm:px-8">
            <div class="flex items-center justify-center gap-6 rounded-2xl border border-violet-200/60 bg-white px-6 py-5 shadow-xl shadow-violet-200/30 sm:gap-10 sm:px-10 sm:py-6 -mt-6 sm:-mt-8" style="background: linear-gradient(145deg, #ffffff 0%, #f5f3ff 100%);">
                <div class="text-center">
                    <div class="text-2xl font-extrabold text-gradient-royal sm:text-3xl">{{ $stats['public_quizzes'] ?? 0 }}</div>
                    <div class="mt-1 text-xs font-medium uppercase tracking-wider text-slate-500">Quizzes</div>
                </div>
                <div class="h-8 w-px bg-gradient-to-b from-transparent via-violet-300 to-transparent"></div>
                <div class="text-center">
                    <div class="text-2xl font-extrabold text-gradient-royal sm:text-3xl">{{ $stats['public_questions'] ?? 0 }}</div>
                ...
                @endif
                @if (count($faculty) > 0)
                    <div class="hidden h-8 w-px bg-gradient-to-b from-transparent via-violet-300 to-transparent sm:block"></div>
    ...
    @if ($v('about') && $v('about', 'bio') && $bio)
        <section class="fade-up bg-white py-16 sm:py-20">
            <div class="mx-auto max-w-3xl px-5 text-center sm:px-8">
                <span class="inline-flex items-center rounded-full bg-gradient-to-r from-violet-100 to-amber-50 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">About us</span>
    ...
    @if (in_array('courses', $instFields))
        <section class="fade-up py-16 sm:py-20" style="background: linear-gradient(180deg, #faf5ff 0%, #fefce8 100%);">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-amber-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-amber-800">What we offer</span>
                ...
                <div class="mt-10 flex flex-wrap justify-center gap-3 sm:gap-4">
                    @foreach (preg_split('/[\n,]+/', $coursesOffered) as $course)
                        ...
                        @if ($course !== '')
                            <div class="rounded-2xl border border-violet-200/80 bg-white px-5 py-3.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-violet-300 hover:shadow-lg hover:shadow-violet-500/10 sm:px-6 sm:py-4 sm:text-base">
                                {{ $course }}
                            </div>
                        @endif
                    @endforeach
    ...
    @if ($v('faculty') && count($faculty) > 0)
        <section class="fade-up bg-white py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-violet-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">Our team</span>
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($faculty as $member)
                        <div class="hover-lift group rounded-2xl border border-violet-100 bg-white p-6 text-center shadow-sm sm:p-8">
                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 via-purple-500 to-amber-500 text-2xl font-bold text-white shadow-lg shadow-violet-500/25 transition group-hover:scale-110">
                                {{ mb_substr($member['name'] ?? '', 0, 1) ?: '?' }}
                            </div>
                            <div class="mt-4">
                                <h3 class="text-lg font-bold text-slate-800">{{ $member['name'] ?? '' }}</h3>
                                @if (!empty($member['role']))
                                    <p class="mt-1 text-sm font-semibold text-violet-600">{{ $member['role'] }}</p>
                                @endif
                                @if (!empty($member['bio']))
    ...
    @if ($v('achievers') && count($selectedStudents) > 0)
        <section class="fade-up relative overflow-hidden py-16 sm:py-20 royal-gradient-hero">
            <div class="royal-glow absolute inset-0"></div>
            <div class="relative mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-amber-400/20 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-amber-200">Success stories</span>
                    <h2 class="mt-4 font-display text-2xl font-bold text-white sm:text-3xl md:text-4xl">Our achievers</h2>
                    <p class="mt-2 text-sm text-violet-200 sm:text-base">Students who made us proud.</p>
                    <div class="mx-auto mt-3 h-[4px] w-14 rounded-full bg-gradient-to-r from-amber-400 to-amber-500"></div>
                </div>
                <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-5">
                    @foreach ($selectedStudents as $student)
                        <div class="hover-lift group flex items-center gap-4 rounded-2xl border border-white/15 bg-white/10 px-5 py-4 backdrop-blur-sm transition hover:bg-white/15 sm:px-6 sm:py-5">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-amber-400/25 text-lg font-bold text-amber-300 transition group-hover:bg-amber-400/35">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if (!empty($student['year']) || !empty($student['post']))
                                    <div class="mt-0.5 text-sm text-violet-200">
                ...
                <div class="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5">
                    @foreach ($galleryImages as $path)
                        @if (\Illuminate\Support\Facades\Storage::disk('public')->exists($path))
                            <div class="gallery-item group overflow-hidden rounded-2xl ring-1 ring-violet-200/50 transition hover:ring-violet-300 hover:shadow-xl">
    ...
    @if (count($instFields) > 0)
        <section class="fade-up py-16 sm:py-20" style="background: linear-gradient(180deg, #f5f3ff 0%, #fefce8 50%);">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-violet-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">Get in touch</span>
                    <h2 class="mt-4 font-display text-2xl font-bold text-slate-900 sm:text-3xl">Visit or contact us</h2>
                    <div class="mx-auto mt-3 section-divider"></div>
                </div>
                <div class="mx-auto mt-10 max-w-3xl overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-xl">
                    <div class="grid gap-0 md:grid-cols-2">
                        <div class="p-6 sm:p-8">
                            <dl class="space-y-5">
                                @if (in_array('name', $instFields))
                                    <div class="flex items-start gap-3">
                                        <div class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-violet-100 text-violet-600">
                                ...
                                @if (in_array('contact', $instFields))
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div><dt class="text-xs font-semibold uppercase tracking-wider text-slate-400">Contact</dt><dd class="mt-0.5"><a href="tel:{{ preg_replace('/[^0-9+]/', '', $centerContact) }}" class="font-semibold text-violet-600 hover:text-violet-700 hover:underline">{{ $centerContact }}</a></dd></div>
                                    </div>
                                @endif
                            </dl>
                        </div>
                        <div class="flex flex-col items-center justify-center gap-4 border-t border-violet-100 p-6 sm:p-8 md:border-l md:border-t-0" style="background: linear-gradient(145deg, #faf5ff 0%, #fefce8 100%);">
                            <div class="text-center">
                            ...
                            <div class="flex flex-col gap-3 w-full max-w-[220px]">
                                ...
                                @if ($hasContact)
                                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', $centerContact) }}"
                                       class="inline-flex items-center justify-center gap-2 rounded-full border-2 border-violet-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-violet-50">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                                        Call now
                                    </a>
                                @endif
    ...
    <section class="fade-up bg-white py-16 sm:py-20">
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
            <div class="text-center">
                <span class="inline-flex items-center rounded-full bg-violet-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">Practice now</span>
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($publicQuizzes as $quiz)
                        <a href="{{ route('public.quizzes.show', $quiz->unique_code) }}"
                           class="hover-lift group flex flex-col rounded-2xl border border-violet-100 bg-white p-5 text-left shadow-sm transition hover:border-violet-200 hover:shadow-xl hover:shadow-violet-500/10 sm:p-6">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <span class="shrink-0 rounded-lg bg-gradient-to-r from-violet-100 to-amber-50 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-violet-700">{{ $quiz->mode }}</span>
                            </div>
                            <div class="mt-auto pt-4 flex items-center justify-between border-t border-violet-100">
                                <span class="text-xs font-medium text-slate-500">{{ $quiz->questions_count ?? 0 }} questions</span>
                                <span class="inline-flex items-center gap-1 text-sm font-bold text-violet-600 group-hover:text-amber-600">
                                    Play
    ...
    @if ($v('social') && !empty($social))
        <section class="py-12" style="background: linear-gradient(180deg, #f5f3ff 0%, #fefce8 100%);">
                    ...
                    @foreach ($social as $label => $url)
                        @if (is_string($url) && $url !== '')
                            <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                               class="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-violet-300 hover:text-violet-600 hover:shadow-md active:scale-[0.97]">
    ...
    <footer class="border-t border-violet-200/60 bg-white py-8">
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
            <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                <div class="flex items-center gap-2">
                    <div class="grid h-7 w-7 place-items-center rounded-lg bg-gradient-to-br from-violet-600 via-purple-600 to-amber-500 text-xs font-bold text-white">Q</div>
