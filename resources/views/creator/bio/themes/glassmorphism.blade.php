...
<head>
    ...
    <style>
        ...
        .hover-lift:hover { transform: translateY(-4px); box-shadow: 0 25px 50px -12px rgba(0,0,0,0.15); }
        .section-divider { width: 48px; height: 3px; border-radius: 3px; background: linear-gradient(90deg, #6366f1, #a78bfa); }
        .gallery-item img { transition: transform 0.4s ease; }
        .gallery-item:hover img { transform: scale(1.05); }
        .glass { background: rgba(255,255,255,0.25); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.3); }
        .glass-dark { background: rgba(0,0,0,0.2); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.15); }
    </style>
</head>
<body class="min-h-screen bg-gradient-to-br from-indigo-100 via-violet-50 to-fuchsia-100 text-slate-800 antialiased font-body">
    @php
    ...
    <header class="glass sticky top-0 z-50 shadow-lg">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
            <a href="{{ url('/') }}" class="flex items-center gap-2.5">
                <div class="grid h-9 w-9 place-items-center rounded-xl bg-white/40 backdrop-blur-md border border-white/50 text-sm font-bold text-indigo-700 shadow-lg">Q</div>
    ...
    <section class="relative overflow-hidden">
        ...
        @else
            <div class="absolute inset-0 bg-gradient-to-br from-indigo-400/40 via-violet-300/30 to-fuchsia-400/40"></div>
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(255,255,255,0.4),transparent)]"></div>
        @endif
        ...
        <div class="absolute bottom-0 left-0 right-0">
            <svg viewBox="0 0 1440 60" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" class="block h-[30px] w-full sm:h-[40px] md:h-[60px]">
                <path d="M0 60h1440V30C1200 55 960 0 720 30S240 55 0 30v30z" fill="#e0e7ff"/>
            </svg>
        </div>
    </section>

    {{-- STATS BAR - Glass --}}
    <section class="relative z-10 -mt-1 bg-transparent">
        <div class="mx-auto max-w-4xl px-5 sm:px-8">
            <div class="glass flex items-center justify-center gap-6 rounded-3xl px-6 py-5 shadow-2xl sm:gap-10 sm:px-10 sm:py-6 -mt-6 sm:-mt-8">
    ...
    @if ($v('about') && $v('about', 'bio') && $bio)
        <section class="fade-up py-16 sm:py-20">
            ...
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                ...
                <div class="mt-10 flex flex-wrap justify-center gap-3 sm:gap-4">
                    @foreach (preg_split('/[\n,]+/', $coursesOffered) as $course)
                        ...
                        @if ($course !== '')
                            <div class="glass rounded-2xl px-5 py-3.5 text-sm font-semibold text-slate-700 transition hover:shadow-xl sm:px-6 sm:py-4 sm:text-base">
                                {{ $course }}
                            </div>
                        @endif
                    @endforeach
            ...
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($faculty as $member)
                        <div class="hover-lift glass group rounded-2xl p-6 text-center shadow-lg sm:p-8">
                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl glass border border-white/40 text-2xl font-bold text-indigo-700 shadow-lg transition group-hover:scale-110">
    ...
    @if ($v('achievers') && count($selectedStudents) > 0)
        <section class="fade-up relative overflow-hidden py-16 sm:py-20">
            <div class="absolute inset-0 bg-gradient-to-br from-indigo-200/50 via-violet-200/40 to-fuchsia-200/50"></div>
            <div class="relative mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    ...
                    <h2 class="mt-4 font-display text-2xl font-bold text-slate-800 sm:text-3xl md:text-4xl">Our achievers</h2>
                    <p class="mt-2 text-sm text-slate-600 sm:text-base">Students who made us proud.</p>
                ...
                <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-5">
                    @foreach ($selectedStudents as $student)
                        <div class="hover-lift glass group flex items-center gap-4 rounded-2xl px-5 py-4 transition hover:shadow-xl sm:px-6 sm:py-5">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-indigo-500/20 text-lg font-bold text-indigo-600 transition group-hover:bg-indigo-500/30">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if (!empty($student['year']) || !empty($student['post']))
                                    <div class="mt-0.5 text-sm text-slate-600">
                                        @if (!empty($student['post']))<span class="font-semibold text-indigo-600">{{ $student['post'] }}</span>@endif
                ...
                <div class="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5">
                    @foreach ($galleryImages as $path)
                        @if (\Illuminate\Support\Facades\Storage::disk('public')->exists($path))
                            <div class="gallery-item glass group overflow-hidden rounded-2xl transition hover:shadow-2xl">
    ...
    @if (count($instFields) > 0)
        <section class="fade-up py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                ...
                <div class="mx-auto mt-10 max-w-3xl overflow-hidden rounded-3xl glass shadow-2xl">
                    <div class="grid gap-0 md:grid-cols-2">
                        ...
                        {{-- Right: CTA panel --}}
                        <div class="flex flex-col items-center justify-center gap-4 border-t border-white/30 glass p-6 sm:p-8 md:border-l md:border-t-0">
                            <div class="text-center">
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($publicQuizzes as $quiz)
                        <a href="{{ route('public.quizzes.show', $quiz->unique_code) }}"
                           class="hover-lift glass group flex flex-col rounded-2xl p-5 text-left shadow-lg transition hover:shadow-2xl sm:p-6">
                    ...
                    @foreach ($social as $label => $url)
                        @if (is_string($url) && $url !== '')
                            <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                               class="inline-flex items-center gap-2 rounded-full glass px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:shadow-xl hover:text-indigo-700 active:scale-[0.97]">
                                @php
    ...
    {{-- FOOTER - Glass --}}
    <footer class="glass border-t border-white/30 py-8">
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
