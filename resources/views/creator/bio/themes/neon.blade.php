...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;800&family=Exo+2:wght@400;500;600;700&display=swap" rel="stylesheet">
    <style>
        .font-display { font-family: 'Orbitron', sans-serif; }
        .font-body    { font-family: 'Exo 2', ui-sans-serif, system-ui, sans-serif; }
        ...
        .text-gradient { background: linear-gradient(135deg, #22d3ee, #a78bfa); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .hover-lift { transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .hover-lift:hover { transform: translateY(-4px); box-shadow: 0 0 30px rgba(34, 211, 238, 0.4), 0 0 60px rgba(167, 139, 250, 0.2); }
        .section-divider { width: 48px; height: 3px; border-radius: 3px; background: linear-gradient(90deg, #22d3ee, #c084fc); box-shadow: 0 0 15px rgba(34, 211, 238, 0.6); }
        .gallery-item img { transition: transform 0.4s ease; }
        .gallery-item:hover img { transform: scale(1.05); }
        .neon-border { border: 1px solid rgba(34, 211, 238, 0.5); box-shadow: 0 0 20px rgba(34, 211, 238, 0.3), inset 0 0 20px rgba(34, 211, 238, 0.05); }
        .neon-glow-cyan { box-shadow: 0 0 20px rgba(34, 211, 238, 0.5); }
        .neon-glow-magenta { box-shadow: 0 0 20px rgba(236, 72, 153, 0.5); }
    </style>
</head>
<body class="min-h-screen bg-slate-950 text-slate-100 antialiased font-body">
    @php
    ...
    {{-- Neon theme: navbar --}}
    <header class="sticky top-0 z-50 border-b border-cyan-500/30 bg-slate-950/95 backdrop-blur-xl neon-border">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
            <a href="{{ url('/') }}" class="flex items-center gap-2.5">
                <div class="grid h-9 w-9 place-items-center rounded-xl bg-cyan-500/20 border border-cyan-400/50 text-sm font-bold text-cyan-300 neon-glow-cyan">Q</div>
                <span class="text-sm font-semibold tracking-tight text-cyan-100">{{ $siteName ?? config('app.name', 'QuizWhiz') }}</span>
            </a>
            <nav class="flex items-center gap-3">
                ...
                @endif
                @auth
                    <a href="{{ route('dashboard') }}" class="rounded-full border border-cyan-500/50 bg-slate-900 px-4 py-2 text-xs font-semibold text-cyan-300 transition hover:neon-glow-cyan">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="rounded-full border border-cyan-500/50 bg-slate-900 px-4 py-2 text-xs font-semibold text-cyan-300 transition hover:neon-glow-cyan">Login</a>
                @endauth
            </nav>
        </div>
    </header>
    ...
    <section class="relative overflow-hidden">
        ...
        @else
            ...
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(34,211,238,0.15),transparent_60%)]"></div>
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_80%_20%,rgba(236,72,153,0.08),transparent)]"></div>
        @endif
        ...
        <div class="relative mx-auto max-w-6xl px-5 pb-16 pt-16 sm:px-8 sm:pb-20 sm:pt-20 md:pb-28 md:pt-24">
            <div class="flex flex-col items-center text-center sm:flex-row sm:items-end sm:text-left sm:gap-8">
                ...
                <div class="mb-6 shrink-0 sm:mb-0">
                    ...
                    @else
                        <div class="flex h-28 w-28 items-center justify-center rounded-2xl border-2 border-cyan-400/60 bg-slate-800/80 text-4xl font-bold text-cyan-300 shadow-2xl neon-glow-cyan sm:h-36 sm:w-36 sm:rounded-3xl sm:text-5xl md:h-40 md:w-40">{{ $initials }}</div>
                    @endif
                </div>
    ...
    <section class="relative z-10 -mt-1 bg-slate-950">
        <div class="mx-auto max-w-4xl px-5 sm:px-8">
            <div class="neon-border flex items-center justify-center gap-6 rounded-2xl bg-slate-900/80 px-6 py-5 sm:gap-10 sm:px-10 sm:py-6 -mt-6 sm:-mt-8">
                <div class="text-center">
                    <div class="text-2xl font-extrabold text-cyan-400 sm:text-3xl neon-glow-cyan">{{ $stats['public_quizzes'] ?? 0 }}</div>
                    <div class="mt-1 text-xs font-medium uppercase tracking-wider text-slate-400">Quizzes</div>
                </div>
                <div class="h-8 w-px bg-cyan-500/40"></div>
                <div class="text-center">
                    <div class="text-2xl font-extrabold text-fuchsia-400 sm:text-3xl neon-glow-magenta">{{ $stats['public_questions'] ?? 0 }}</div>
                    <div class="mt-1 text-xs font-medium uppercase tracking-wider text-slate-400">Questions</div>
                </div>
                @if (count($selectedStudents) > 0)
                    <div class="h-8 w-px bg-cyan-500/40"></div>
                    <div class="text-center">
                        <div class="text-2xl font-extrabold text-lime-400 sm:text-3xl">{{ count($selectedStudents) }}+</div>
                        <div class="mt-1 text-xs font-medium uppercase tracking-wider text-slate-400">Selections</div>
                    </div>
                @endif
                @if (count($faculty) > 0)
                    <div class="hidden h-8 w-px bg-cyan-500/40 sm:block"></div>
                    <div class="hidden text-center sm:block">
                        <div class="text-2xl font-extrabold text-cyan-400 sm:text-3xl">{{ count($faculty) }}</div>
                        <div class="mt-1 text-xs font-medium uppercase tracking-wider text-slate-400">Faculty</div>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if ($v('about') && $v('about', 'bio') && $bio)
        <section class="fade-up bg-slate-900 py-16 sm:py-20">
            <div class="mx-auto max-w-3xl px-5 text-center sm:px-8">
                <span class="inline-flex items-center rounded-full bg-cyan-500/20 border border-cyan-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-cyan-300">About us</span>
                ...
                <p class="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-slate-300 whitespace-pre-line sm:text-lg sm:leading-8">{{ $bio }}</p>
            </div>
        </section>
    @endif
    ...
    @if (in_array('courses', $instFields))
        <section class="fade-up bg-slate-950 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-fuchsia-500/20 border border-fuchsia-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-fuchsia-300">What we offer</span>
                    <h2 class="mt-4 font-display text-2xl font-bold text-slate-100 sm:text-3xl">Courses &amp; programmes</h2>
                    <div class="mx-auto mt-3 section-divider"></div>
                </div>
                <div class="mt-10 flex flex-wrap justify-center gap-3 sm:gap-4">
                    @foreach (preg_split('/[\n,]+/', $coursesOffered) as $course)
                        ...
                        @if ($course !== '')
                            <div class="neon-border rounded-2xl bg-slate-900/80 px-5 py-3.5 text-sm font-semibold text-slate-200 transition hover:neon-glow-cyan sm:px-6 sm:py-4 sm:text-base">
                                {{ $course }}
                            </div>
                        @endif
                    @endforeach
    ...
    @if ($v('faculty') && count($faculty) > 0)
        <section class="fade-up bg-slate-900 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-cyan-500/20 border border-cyan-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-cyan-300">Our team</span>
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($faculty as $member)
                        <div class="hover-lift neon-border group rounded-2xl bg-slate-800/60 p-6 text-center sm:p-8">
                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-cyan-400/50 bg-cyan-500/20 text-2xl font-bold text-cyan-300 neon-glow-cyan transition group-hover:scale-110">
    ...
    @if ($v('achievers') && count($selectedStudents) > 0)
        <section class="fade-up relative overflow-hidden bg-slate-950 py-16 sm:py-20">
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(34,211,238,0.08),transparent)]"></div>
            <div class="relative mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-lime-500/20 border border-lime-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-lime-300">Success stories</span>
                ...
                <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-5">
                    @foreach ($selectedStudents as $student)
                        <div class="hover-lift neon-border group flex items-center gap-4 rounded-2xl bg-slate-900/80 px-5 py-4 transition hover:neon-glow-magenta sm:px-6 sm:py-5">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-fuchsia-500/20 text-lg font-bold text-fuchsia-300 transition group-hover:bg-fuchsia-500/30">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if (!empty($student['year']) || !empty($student['post']))
                                    <div class="mt-0.5 text-sm text-slate-400">
                                        @if (!empty($student['post']))<span class="font-semibold text-fuchsia-300">{{ $student['post'] }}</span>@endif
    ...
    @if ($v('images') && $v('images', 'gallery') && count($galleryImages) > 0)
        <section class="fade-up bg-slate-900 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-cyan-500/20 border border-cyan-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-cyan-300">Life at our institute</span>
                    <h2 class="mt-4 font-display text-2xl font-bold text-slate-100 sm:text-3xl">Gallery</h2>
                    <p class="mt-2 text-sm text-slate-400 sm:text-base">Classroom, events &amp; more.</p>
                    <div class="mx-auto mt-3 section-divider"></div>
                </div>
                <div class="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5">
                    @foreach ($galleryImages as $path)
                        @if (\Illuminate\Support\Facades\Storage::disk('public')->exists($path))
                            <div class="gallery-item neon-border group overflow-hidden rounded-2xl transition hover:neon-glow-cyan">
    ...
    @if (count($instFields) > 0)
        <section class="fade-up bg-slate-950 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-fuchsia-500/20 border border-fuchsia-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-fuchsia-300">Get in touch</span>
                    <h2 class="mt-4 font-display text-2xl font-bold text-slate-100 sm:text-3xl">Visit or contact us</h2>
                    <div class="mx-auto mt-3 section-divider"></div>
                </div>

                <div class="mx-auto mt-10 max-w-3xl overflow-hidden rounded-3xl neon-border bg-slate-900/80 shadow-2xl">
    ...
    {{-- QUIZZES SECTION - Neon --}}
    <section class="fade-up bg-slate-900 py-16 sm:py-20">
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
            <div class="text-center">
                <span class="inline-flex items-center rounded-full bg-cyan-500/20 border border-cyan-400/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-cyan-300">Practice now</span>
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($publicQuizzes as $quiz)
                        <a href="{{ route('public.quizzes.show', $quiz->unique_code) }}"
                           class="hover-lift neon-border group flex flex-col rounded-2xl bg-slate-800/60 p-5 text-left transition hover:neon-glow-cyan sm:p-6">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0 flex-1">
                                    <h3 class="font-bold text-slate-100 break-words group-hover:text-cyan-300 leading-snug">{{ $quiz->title }}</h3>
                                    @if ($quiz->description)
                                        <p class="mt-1.5 line-clamp-2 text-sm leading-relaxed text-slate-500">{{ $quiz->description }}</p>
                                    @endif
                                </div>
                                <span class="shrink-0 rounded-lg bg-cyan-500/20 border border-cyan-400/40 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-cyan-300">{{ $quiz->mode }}</span>
                            </div>
                            <div class="mt-auto pt-4 flex items-center justify-between border-t border-cyan-500/30">
                                ...
                                <span class="inline-flex items-center gap-1 text-sm font-bold text-cyan-400 group-hover:text-cyan-300">
                                    Play
                    ...
                    @foreach ($social as $label => $url)
                        @if (is_string($url) && $url !== '')
                            <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                               class="inline-flex items-center gap-2 rounded-full neon-border bg-slate-900/80 px-5 py-2.5 text-sm font-semibold text-cyan-200 transition hover:neon-glow-cyan active:scale-[0.97]">
                                @php
        ...
        <div class="mx-auto max-w-6xl px-5 sm:px-8">
            <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                <div class="flex items-center gap-2">
                    <div class="grid h-7 w-7 place-items-center rounded-lg bg-cyan-500/20 border border-cyan-400/50 text-xs font-bold text-cyan-300 neon-glow-cyan">Q</div>
                    <span class="text-sm font-semibold text-slate-300">{{ $siteName ?? config('app.name', 'QuizWhiz') }}</span>
                </div>
