...
<head>
    ...
    <style>
        ...
        .text-gradient { background: linear-gradient(135deg, #ec4899, #8b5cf6, #06b6d4); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .hover-lift { transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .hover-lift:hover { transform: translateY(-4px); box-shadow: 0 20px 40px -15px rgba(139, 92, 246, 0.4); }
        .section-divider { width: 56px; height: 4px; border-radius: 4px; background: linear-gradient(90deg, #ec4899, #8b5cf6, #06b6d4); }
        .gallery-item img { transition: transform 0.4s ease; }
        .gallery-item:hover img { transform: scale(1.05); }
        .gradient-card { background: linear-gradient(135deg, rgba(255,255,255,0.9) 0%, rgba(245,240,255,0.95) 100%); border: 1px solid rgba(139,92,246,0.2); }
        .gradient-bg-1 { background: linear-gradient(180deg, #fdf2f8 0%, #f5f3ff 50%, #ecfeff 100%); }
        .gradient-bg-2 { background: linear-gradient(180deg, #f5f3ff 0%, #fdf2f8 50%); }
    </style>
</head>
<body class="min-h-screen bg-gradient-to-b from-pink-50 via-violet-50 to-cyan-50 text-slate-800 antialiased font-body">
    @php
    ...
    {{-- Gradient theme: navbar --}}
    <header class="sticky top-0 z-50 border-b border-violet-200/50 bg-white/80 backdrop-blur-xl">
        <div class="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-8">
            <a href="{{ url('/') }}" class="flex items-center gap-2.5">
                <div class="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-pink-500 via-violet-500 to-cyan-500 text-sm font-bold text-white shadow-lg shadow-violet-500/30">Q</div>
    ...
    <section class="relative overflow-hidden">
        ...
        @else
            <div class="absolute inset-0 bg-gradient-to-br from-pink-500/90 via-violet-600/90 to-cyan-600/90"></div>
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(255,255,255,0.15),transparent)]"></div>
        @endif
        ...
        <div class="relative mx-auto max-w-6xl px-5 pb-16 pt-16 sm:px-8 sm:pb-20 sm:pt-20 md:pb-28 md:pt-24">
            <div class="flex flex-col items-center text-center sm:flex-row sm:items-end sm:text-left sm:gap-8">
                ...
                <div class="mb-6 shrink-0 sm:mb-0">
                    ...
                    @else
                        <div class="flex h-28 w-28 items-center justify-center rounded-2xl border-4 border-white/30 bg-gradient-to-br from-pink-400 via-violet-500 to-cyan-500 text-4xl font-bold text-white shadow-2xl sm:h-36 sm:w-36 sm:rounded-3xl sm:text-5xl md:h-40 md:w-40">{{ $initials }}</div>
                    @endif
                </div>
        ...
        <div class="absolute bottom-0 left-0 right-0">
            <svg viewBox="0 0 1440 60" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" class="block h-[30px] w-full sm:h-[40px] md:h-[60px]">
                <path d="M0 60h1440V30C1200 55 960 0 720 30S240 55 0 30v30z" fill="#fdf2f8"/>
            </svg>
        </div>
    </section>

    {{-- STATS BAR - Gradient --}}
    <section class="relative z-10 -mt-1 gradient-bg-1">
        <div class="mx-auto max-w-4xl px-5 sm:px-8">
            <div class="gradient-card flex items-center justify-center gap-6 rounded-3xl px-6 py-5 shadow-2xl shadow-violet-200/40 sm:gap-10 sm:px-10 sm:py-6 -mt-6 sm:-mt-8">
                <div class="text-center">
                    <div class="text-2xl font-extrabold text-gradient sm:text-3xl">{{ $stats['public_quizzes'] ?? 0 }}</div>
                    <div class="mt-1 text-xs font-medium uppercase tracking-wider text-slate-500">Quizzes</div>
                </div>
                <div class="h-8 w-px bg-slate-200"></div>
                <div class="text-center">
                    <div class="text-2xl font-extrabold text-gradient sm:text-3xl">{{ $stats['public_questions'] ?? 0 }}</div>
    ...
    @if ($v('about') && $v('about', 'bio') && $bio)
        <section class="fade-up gradient-bg-2 py-16 sm:py-20">
            <div class="mx-auto max-w-3xl px-5 text-center sm:px-8">
                <span class="inline-flex items-center rounded-full bg-gradient-to-r from-pink-100 to-violet-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">About us</span>
    ...
    @if (in_array('courses', $instFields))
        <section class="fade-up gradient-bg-1 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-gradient-to-r from-violet-100 to-cyan-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">What we offer</span>
                ...
                <div class="mt-10 flex flex-wrap justify-center gap-3 sm:gap-4">
                    @foreach (preg_split('/[\n,]+/', $coursesOffered) as $course)
                        ...
                        @if ($course !== '')
                            <div class="gradient-card rounded-2xl px-5 py-3.5 text-sm font-semibold text-slate-700 transition hover:shadow-xl hover:shadow-violet-200/50 sm:px-6 sm:py-4 sm:text-base">
                                {{ $course }}
                            </div>
                        @endif
                    @endforeach
    ...
    @if ($v('faculty') && count($faculty) > 0)
        <section class="fade-up gradient-bg-2 py-16 sm:py-20">
            <div class="mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-gradient-to-r from-pink-100 to-violet-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-violet-700">Our team</span>
                ...
                <div class="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                    @foreach ($faculty as $member)
                        <div class="hover-lift gradient-card group rounded-2xl p-6 text-center shadow-lg sm:p-8">
                            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-500 via-violet-500 to-cyan-500 text-2xl font-bold text-white shadow-lg shadow-violet-500/30 transition group-hover:scale-110">
    ...
    @if ($v('achievers') && count($selectedStudents) > 0)
        <section class="fade-up relative overflow-hidden bg-gradient-to-br from-violet-600 via-fuchsia-600 to-cyan-600 py-16 sm:py-20">
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(255,255,255,0.15),transparent)]"></div>
            <div class="relative mx-auto max-w-6xl px-5 sm:px-8">
                <div class="text-center">
                    <span class="inline-flex items-center rounded-full bg-white/20 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-white">Success stories</span>
                ...
                <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-5">
                    @foreach ($selectedStudents as $student)
                        <div class="hover-lift group flex items-center gap-4 rounded-2xl border border-white/20 bg-white/10 px-5 py-4 backdrop-blur-sm transition hover:bg-white/20 sm:px-6 sm:py-5">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-white/20 text-lg font-bold text-white transition group-hover:bg-white/30">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if (!empty($student['year']) || !empty($student['post']))
                                    <div class="mt-0.5 text-sm text-slate-400">
                                        @if (!empty($student['post']))<span class="font-semibold text-cyan-200">{{ $student['post'] }}</span>@endif
