...
<div class="space-y-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Edit Contest</h1>
            <p class="mt-1 text-sm text-slate-600">
                @if ($contest->join_code)
                    Join code: <code class="rounded bg-slate-100 px-2 py-1">{{ $contest->join_code }}</code>
                @else
                    No join code (mode: {{ $contest->join_mode }})
                @endif
            </p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <form method="POST" action="{{ route('creator.contests.update', $contest) }}" class="space-y-4">
            @csrf
            @method('PUT')
