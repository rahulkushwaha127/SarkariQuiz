    ...
    <div x-data="{ activeSection: '{{ request()->get('section', 'branding') }}' }" class="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-6">
        ...
        <div class="card p-0 overflow-hidden min-h-[30rem] h-[calc(100vh-11rem)] lg:h-[calc(100vh-11rem)] flex flex-col">
            <nav class="flex flex-col flex-1 overflow-y-auto custom-scrollbar">
                ...
                <button 
                    @click="activeSection = 'stripe'" 
                    :class="activeSection === 'stripe' ? 'bg-primary-600 text-white' : 'bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-slate-700'"
                    class="w-full px-4 py-3 flex items-center justify-between transition-colors border-t border-gray-200 dark:border-slate-700"
                >
                    <span class="font-medium">{{ __('Stripe') }}</span>
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                </button>
        ...
        <div>
                ...
                <form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4 mb-6">
                        <div>
                            ...
                            <input type="text" name="app_name" value="{{ old('app_name', $settings['app_name'] ?? config('app.name')) }}" class="w-full px-3 py-2 rounded-xl border bg-white dark:bg-slate-800 text-gray-900 dark:text-white">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">{{ __('App Description') }}</label>
                            <input type="text" name="app_description" value="{{ old('app_description', $settings['app_description'] ?? '') }}" class="w-full px-3 py-2 rounded-xl border bg-white dark:bg-slate-800 text-gray-900 dark:text-white" placeholder="{{ __('Enter app description') }}">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">{{ __('Footer Text') }}</label>
                            <textarea name="footer_text" rows="2" class="w-full px-3 py-2 rounded-xl border bg-white dark:bg-slate-800 text-gray-900 dark:text-white" placeholder="© {{ date('Y') }} {{ config('app.name') }}. {{ __('All rights reserved.') }}">{{ old('footer_text', $settings['footer_text'] ?? '') }}</textarea>
                            <p class="text-xs text-gray-500 dark:text-slate-500 mt-1">{{ __('This text will appear at the bottom of all emails. Leave empty to use default.') }}</p>
                        </div>
                    </div>
            ...
            <!-- Stripe Section -->
            <div x-show="activeSection === 'stripe'" x-cloak class="card" x-data="{ stripeEnabled: {{ !empty($settings['enabled']) && $settings['enabled'] == '1' ? 'true' : 'false' }} }">
                <div class="mb-6 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Stripe') }}</h3>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input 
                            type="checkbox" 
                            x-model="stripeEnabled"
                            @change="document.getElementById('stripe_enabled_input').value = $event.target.checked ? '1' : '0';"
                            class="sr-only peer"
                            :checked="stripeEnabled"
                        >
                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary-300 dark:peer-focus:ring-primary-800 rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-primary-600"></div>
                        <span class="ml-3 text-sm font-medium text-gray-700 dark:text-slate-300" x-text="stripeEnabled ? '{{ __('Enabled') }}' : '{{ __('Disabled') }}'"></span>
                    </label>
                </div>
                
                <form method="POST" action="{{ route('admin.settings.update') }}" id="stripe-form">
                    @csrf
                    <input type="hidden" name="section" value="stripe">
                    <input type="hidden" name="stripe_enabled" id="stripe_enabled_input" value="{{ !empty($settings['enabled']) && $settings['enabled'] == '1' ? '1' : '0' }}">
                    
                    <div class="space-y-4 mb-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">{{ __('Publishable Key') }}</label>
                            <input type="text" name="stripe_key" value="{{ old('stripe_key', $settings['key'] ?? config('services.stripe.key')) }}" class="w-full px-3 py-2 rounded-xl border bg-white dark:bg-slate-800 text-gray-900 dark:text-white" placeholder="pk_test_...">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">{{ __('Secret Key') }}</label>
                            <input type="password" name="stripe_secret" value="{{ old('stripe_secret', $settings['secret'] ?? config('services.stripe.secret')) }}" class="w-full px-3 py-2 rounded-xl border bg-white dark:bg-slate-800 text-gray-900 dark:text-white" placeholder="sk_test_...">
                        </div>
                    </div>
