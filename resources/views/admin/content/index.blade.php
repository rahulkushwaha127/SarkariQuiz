...
<div class="space-y-6">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-600">Browse question JSON files in the <code class="rounded bg-slate-100 px-1 py-0.5 text-xs">content/</code> folder. Subject → Language → Topic → JSON files.</p>
    </div>
    ...
    (function() {
        ...
        if (topicEl) {
            topicEl.addEventListener('change', function() { form.submit(); });
        }
    })();
    </script>
    ...
    @if($subject !== '' && $language !== '')
        <div class="flex flex-wrap items-center gap-2 text-sm text-slate-600">
            <span>Path:</span>
            <span class="font-mono rounded bg-slate-100 px-2 py-1">content/{{ $subject }}/{{ $language }}{{ $topic !== '' ? '/' . $topic : '' }}</span>
            @if($totalQuestions > 0)
                            ...
                            @foreach($files as $f)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <a href="{{ route('admin.content.file', ['subject' => $subject, 'language' => $language, 'topic' => $topic, 'file' => $f['name']]) }}"
