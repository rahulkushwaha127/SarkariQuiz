...
@section('content')
    <div class="flex flex-col min-h-screen justify-center items-center p-4">
        ...
        <div
            ...
            @if ($data['question']->explanation && getUserSettings('enable_question_explanations', $data['question']->quiz->user_id))
                <div id="question-explanation"
                    class="border border-gray-200 bg-gray-50 rounded-xl p-4 mt-4 shadow-sm d-none">
                    <div class="flex items-start gap-3 text-left">
                        <div class="pt-1">
                            <svg class="w-6 h-6 text-yellow-500" fill="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M12 2a7 7 0 00-7 7c0 2.61 1.67 4.25 2.72 5.33.58.6 1.28 1.33 1.28 1.67v1a1 1 0 001 1h4a1 1 0 001-1v-1c0-.34.7-1.07 1.28-1.67C17.33 13.25 19 11.61 19 9a7 7 0 00-7-7zm-2 18a1 1 0 100 2h4a1 1 0 100-2h-4z" />
                            </svg>
                        </div>
                        <div class="text-left w-full">
                            <h2 class="font-semibold text-lg sm:text-xl text-gray-800 text-left">{{ __('messages.quiz.explanation') }}</h2>
                            <p class="mt-2 text-gray-700 text-sm sm:text-base leading-relaxed text-left">
        ...
        @php
            ...
            $dataAttributes = [
                ...
                'data-quiz-mode' => $data['question']->quiz->quiz_mode,
            ];
                ...
                <div class="grid grid-cols-1 gap-3 mb-8" id="answers-container"
                    ...
                    @foreach ($data['question']->answers as $answer)
                        <label for="answer-{{ $answer->id }}"
                            class="flex justify-between items-center px-5 py-4 border border-gray-300 rounded-lg cursor-pointer transition-all duration-300 {{ $answer->is_correct ? 'Tg7LZd' : '' }}"
                            style="background-color: #f3f4f6;">
                            @if ($data['isMultipleChoice'])
                                <input type="checkbox" id="answer-{{ $answer->id }}" class="hidden peer "
