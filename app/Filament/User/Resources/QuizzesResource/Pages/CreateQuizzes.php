...
class CreateQuizzes extends CreateRecord
{
    ...
    protected function handleRecordCreation(array $data): Model
    {
        ...
        **Additional Instruction for Study Mode:**
                ...
                try {
                    $geminiResponse = Http::withHeaders([
                        ...
                        'contents' => [
                            [
                                'parts' => [
                                    ['text' => mb_convert_encoding($prompt, 'UTF-8', 'UTF-8')],
                                ],
                            ],
                        ],
                    ]);
                    ...
                    Notification::make()
                        ->danger()
                        ->title('Gemini API error: ' . $e->getMessage())
                        ->send();
                    $this->halt();
                }
        ...
        if ($addManuallyQuestions) {
            $quizText = $data['custom_questions'] ?? [];
        }
        ...
        if ($quizText) {
            ...
            if (!$addManuallyQuestions) {
                ...
                $quizQuestions = $quizText;
            }
            if ($addManuallyQuestions) {
                $invalidIndex = collect($quizQuestions)->search(function ($question) use ($input) {
                    $correctCount = collect($question['answers'])->where('is_correct', true)->count();

                    if ($input['quiz_type'] == Quiz::SINGLE_CHOICE) {
            ...
            $input['max_questions'] = count($quizQuestions);
            ...
            if (is_array($quizQuestions)) {
                foreach ($quizQuestions as $index => $question) {
                    ...
                    $questionText = $question['question'] ?? ($question['title'] ?? null);

                    if ($questionText && isset($question['answers']) && is_array($question['answers'])) {
                        ...
                        foreach ($question['answers'] as $answer) {
                            ...
                            Answer::create([
                                ...
                                'title' => $answer['title'],
                                'is_correct' => $addManuallyQuestions ? $answer['is_correct'] : $isCorrect,
                            ]);
                        }
                    }
                }
